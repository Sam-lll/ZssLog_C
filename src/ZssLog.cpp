...
FILE* g_fp; 
void WINAPI ZssLog(unsigned char* pszSubName)
{
	char tmp[260] = {0};
	char logdir[260] = {0};
	int auto_delete = 0;
...
int WINAPI Open(const char* log_base_dir, int enable_auto_delete, int auto_delete_keep_days)
{
	char pszIt[260] = {0};
	int i = 0, n = 0;
	TIME stTime;
...
void WINAPI LogInfo(const char* lpszFormat, ...)
{
	va_list arg;
	if (!g_log_enable)
		return;
...
void WINAPI LogError(const char* lpszFormat, ...)
{
	va_list arg;
	if (!g_log_enable)
		return;
...
void WINAPI LogWarn(const char* lpszFormat, ...)
{
	va_list arg;
	if (!g_log_enable)
		return;
...
void WINAPI LogDebug(const char* lpszFormat, ...)
{
	va_list arg;
	if (!g_log_enable)
		return;
...
void WINAPI LogException(const char* lpszFormat, ...)
{
	va_list arg;
	if (!g_log_enable)
		return;
...
void WINAPI Logv(int type, const char* lpszFormat, va_list arg)
{
...
void WINAPI CheckChangeFile(/*TIME *st*/int curr_stamp, int day_sec)
{
...
void WINAPI WriteRecord(const char *line)
{
	int nWrite = 0;
